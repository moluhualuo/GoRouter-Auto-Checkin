## 🚀 GoRouter 自动签到领额度（GitHub Actions）

这是一个基于 GitHub Actions 的自动化脚本，用于定时登录自动签到 [gorouter.app](https://gorouter.app) 服务。
...
| Secret 名称         | 是否必填 | 说明                                              |
|---------------------|----------|---------------------------------------------------|
| USER_ID            | ❌ 可选  | 无需填写；脚本会通过 SESSION 自动识别当前用户。若已知可保留作兼容配置。 |
